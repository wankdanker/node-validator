...
Validator.prototype.isFutureDate = Validator.prototype.isDateAfter = function(compareDate) {
    if (!isDate(this.str)) {
        return this.error(this.msg || 'Not a date');
    }
    
    var ts = (new Date(this.str)).getTime();
    var now = (compareDate) ? (new Date(compareDate)).getTime() : +new Date;
    
    if (ts < now) {
        return this.error(this.msg || 'Not a future date');
    }
    
    return this;
};

Validator.prototype.isPastDate = Validator.prototype.isDateBefore = function(compareDate) {
    if (!isDate(this.str)) {
        return this.error(this.msg || 'Not a date in the future');
    }
    ...
    if (ts > now) {
        return this.error(this.msg || 'Not a date in the past');
    }
...
function isDate(str) {
    ...
    if (   dt.getFullYear()  != parseInt(result[3],10)
        || dt.getMonth() + 1 != parseInt(result[1],10)
        || dt.getDate()      != parseInt(result[2],10)
    ) {
        return false;
    }
